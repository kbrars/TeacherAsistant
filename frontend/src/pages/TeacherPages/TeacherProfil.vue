<template>
  <q-page >
    ...
    <q-form @submit.prevent="updateProfile"  :style="{ marginBottom: '20px' }" method="POST">
      ...
      &nbsp;
      <q-input  :style="{ marginBottom: '20px' }" filled label="Address" v-model="userInfo.address" :readonly="!isEditing"/>
      <q-input  :style="{ marginBottom: '20px' }" filled label="Phone" v-model="userInfo.phone" :readonly="!isEditing"/>
      <q-btn class="mb-4" @click="changePasswordButton" color="blue-9" label="Change the Password" :style="{width:'100%',  marginBottom: '-8px' }"/>
      <q-btn class="mb-4"  v-if="isEditing" color="green-9" label="Save" type="submit" :style="{width:'100%' , marginTop: '18px'}"/>
    </q-form>
    <q-btn v-if="!isEditing" @click="startEditing" color="blue-9" label="Edit" class="full-width-btn" :style="{width:'100%'}"/>
      ...
      <q-card style="min-width: 400px; max-width: 90vw;">
        ...
        <q-card-section>
          <q-form @submit.prevent="verifyCurrentPassword">
            <q-input v-model="currentPassword" label="Current Password" type="password" :rules="[requiredRule]" dense required />
      ...
      <q-card style="min-width: 400px; max-width: 90vw;">
        ...
        <q-card-section>
          <q-form @submit.prevent="updatePassword">
            <q-input v-model="newPassword" label="New Password" type="password" :rules="[requiredRule]" dense required />
            <q-input v-model="confirmNewPassword" label="Confirm New Password" type="password" :rules="[requiredRule, passwordMatchRule]" dense required />
            <q-btn flat label="No" color="negative" v-close-popup />
